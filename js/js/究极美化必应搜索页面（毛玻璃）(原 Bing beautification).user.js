// ==UserScript==
// @name         究极美化必应搜索页面（毛玻璃）(原 Bing beautification)
...
(function() {
    ...
    // @require     https://greasyfork.org/scripts/453994-bing-beautification-store/code/Bing%20beautification%20store.js
    ...
    if (window.location.href.indexOf("cn.bing.com/search") > -1) {
        ...
        //全局图片
        function fullPicSet() {
            //document.querySelector("html").style.background = `url(${$.cookie('picUrl')}) 0% 0% / 111% no-repeat fixed`
            document.querySelector("html").style.background = `url(${$.cookie('picUrl')}) 0 / cover fixed`
    }
        ...
        var qna_algo=document.getElementsByClassName('qna_algo')
        var tab_head_HeroTab = document.getElementsByClassName('HeroTab')
        ...
        var length1
        if(b_context){
            length1= b_context.children.length
        }
        // header背景
    ...
    //    temp.style.backgroundAttachment = "fixed"
       //     temp.style.backgroundSize = "100%"
        ...
        function fixotherQues() {
            //qna_algo 白溢出问题
            if(qna_algo){
                for(let i=0;i<=qna_algo.length-1;i++){
                    qna_algo[i].children[0].style=""
                }
            }
            //tab_head_HeroTab 百科推荐栏
           /* if(tab_head_HeroTab[0]){
                if(tab_head_HeroTab[0].children){
                tab_head_HeroTab[0].children[0].style.background="transparent"
                tab_head_HeroTab[0].children[2].style.background="transparent"
                }
            }*/
