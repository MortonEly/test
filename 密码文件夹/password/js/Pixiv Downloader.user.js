...
/>`;
  ...
  const lang = document.documentElement.getAttribute('lang')?.toLowerCase() || 'en';
  const i18nLib = {
      en: {
          bookmarks: 'Bookmarks',
          ...
          'modal_filename_content.use_fsa': 'FileSystemAccess API',
          'modal_filename_content.show_dir_placeholder': 'Root directory',
          'modal_filename_content.change_fsa_dir': 'Change',
          'modal_filename_content.conflict_action': 'ConflictAction: ',
          'modal_filename_content.conflict_action_uniquify': 'Uniquify',
          'modal_filename_content.conflict_action_overwrite': 'Overwrite',
          'modal_filename_content.conflict_action_prompt': 'Prompt',
          tabs_nav_history: 'History',
          'modal_history_content.import_btn': 'Import (Replace)',
          'modal_history_content.merge_btn': 'Import (Merge)',
      ...
      'zh-cn': {
          ...
          tags_lang: '标签语言：',
          tags_tips: '{artist}:作者, {artistID}:作者ID, {title}:作品标题, {id}:作品ID, {page}:页码, {tags}:作品标签。',
          ...
          filename: '文件名：',
          filename_tips: '你的名字？',
          'modal_filename_content.use_fsa': '使用FileSystemAccess API',
          'modal_filename_content.show_dir_placeholder': '根文件夹名',
          'modal_filename_content.change_fsa_dir': '更改',
          'modal_filename_content.conflict_action': '文件名重复时：',
          'modal_filename_content.conflict_action_uniquify': '重命名',
          'modal_filename_content.conflict_action_overwrite': '覆盖',
          'modal_filename_content.conflict_action_prompt': '提示',
          tabs_nav_history: '历史记录',
          'modal_history_content.import_btn': '导入记录（替换）',
          'modal_history_content.merge_btn': '导入记录（合并）',
  ...
  const modalHtml = {
      upgradeMsgTitle: `<h3>Pixiv Downloader ${config.get('version')}</h3>`,
      upgradeMsgContent: `      <div class="pdl-changelog">
  <p>夏日炎炎，记得补充可乐。</p>
  <ul>
    <li>调整批量下载获取作品信息方式。</li>
    <li>修复一些画师名导致保存失败的问题。</li>
    <li>修复一些作品名导致保存异常的问题。</li>
  </ul>
</div>`,
  ...
  <div>
    ...
    <div class="pdl-input-wrap">
      ...
      <input type="text" id="pdlfilename" placeholder="${i18n('filename_tips')}" required maxlength="100" />
  ...
  <p style="font-size: 14px; margin: 0.5em 0">${i18n('tags_tips')}</p>
  ...
  <div ${env.isFileSystemAccessAvaliable() ? '' : 'class="pdl-unavailable"'}>
    <div style="display: flex; justify-content: space-between; align-items: center; margin: 12px 0; gap: 12px">
      <label class="pdl-options" style="padding: 0.6em 4px">
        <span style="font-weight: 700">${i18n('modal_filename_content.use_fsa')}</span>
      ...
      <div class="pdl-input-wrap" style="flex: 1; margin: 0">
        <input id="pdl-fsa-show-directory" type="text" placeholder="${i18n('modal_filename_content.show_dir_placeholder')}" style="font-size: 14px; padding: 8px 0.5em; line-height: 1.15" disabled/>
      </div>
      <button id="pdl-fsa-change-directory" class="pdl-dialog-button primary">${i18n('modal_filename_content.change_fsa_dir')}</button>
    </div>
    <div class="tags-option">
      <span class="tags-title">${i18n('modal_filename_content.conflict_action')}</span>
      <div class="tags-content">
        <div class="tags-item">
          <input class="pdl-option-conflict" type="radio" name="conflict_action" id="action_uniquify" value="uniquify"/>
          <label for="action_uniquify">${i18n('modal_filename_content.conflict_action_uniquify')}</label>
        </div>
        <div class="tags-item">
          <input class="pdl-option-conflict" type="radio" name="conflict_action" id="action_overwrite" value="overwrite"/>
          <label for="action_overwrite">${i18n('modal_filename_content.conflict_action_overwrite')}</label>
        </div>
        <div class="tags-item">
          <input class="pdl-option-conflict" type="radio" name="conflict_action" id="action_prompt" value="prompt"/>
          <label for="action_prompt">${i18n('modal_filename_content.conflict_action_prompt')}</label>
        </div>
      </div>
    </div>
  </div>
...
<div>
  ...
  /><button id="pdl-merge-btn" class="btn-history pdl-dialog-button primary">
    ${i18n('modal_history_content.merge_btn')}
  </button>
</div>
<div>
...
onmessage = (evt) => {
  ...
  function createService() {
      async function _requestJson(url, init) {
          ...
          if (!res.ok)
              throw new Error('Fail to fetch:' + url + ', code:' + res.status, { cause: '429' });
          const data = await res.json();
          if (data.error)
              throw new Error('JSON return error: ' + data.message, { cause: 'json error' });
          return data.body;
      }
      async function _getDeps(url) {
          return fetch(url).then((res) => {
              if (res.ok)
                  return res.text();
              throw new Error(`Fetch dependency ${url} failed: ${res.status} ${res.statusText}`);
          });
      }
      return {
          async getJson(url) {
  ...
  function createConverter() {
      ...
      const convertTo = {
              ...
              return new Promise((resolve, reject) => {
                  ...
                  frames.forEach((frame, idx) => {
                      ...
                      data.push(new Promise((resolve, reject) => {
                          canvas.toBlob((blob) => {
                              if (!blob)
                                  return reject(new Error('canvas.toBlob() failed: ' + idx));
          ...
          png: (frames, convertMeta) => {
              return new Promise((resolve, reject) => {
                  ...
                  if (!context)
                      return reject(new Error('Can not get canvas context'));
                  const data = [];
                  ...
                  worker.onmessage = function (e) {
                      ...
                      logger.timeEnd(convertMeta.id);
                      if (!e.data) {
                          return reject(new Error('Failed to get png data. ' + convertMeta.id));
                      }
      ...
      function createImgElements(zip) {
          const eles = [];
          zip.forEach((_, file) => {
      ...
      const convert = (convertMeta) => {
          ...
          if (!newFolder)
              return reject(new Error('Can not get new root folder'));
          newFolder
              .loadAsync(source.data)
              .then(createImgElements)
              .then((imgEles) => {
              zip.remove(id);
              frames = imgEles;
              if (convertMeta.isAborted) {
                  ...
                  if ((format === 'gif' || format === 'png') && depsStatus[format].loaded !== LoadStatus.loaded) {
                      switch (depsStatus[format].loaded) {
                          ...
                          case LoadStatus.loading:
                              ...
                              break;
                          default:
                              throw new Error('Invalid deps status.');
                      }
                  }
                  else {
                      queue.push(meta);
  ...
  async function getFilenameHandle(dirHandle, filename) {
      ...
      if (conflictAction === 'prompt') {
          ...
          throw new Error('Oops, you have too many duplicate files.');
      }
  }
  ...
  function rejectCachedSave() {
      cachedSaveProcess.forEach(([, downloadMeta]) => downloadMeta.reject());
  ...
  const _saveWithSubpath = (blob, downloadMeta) => {
      ...
      const request = {
          ...
          onerror: (error) => {
              if (downloadMeta.state !== 0 ) {
                  logger.error(error, downloadMeta.source.path);
                  downloadMeta.reject(new Error('Download error when saving'));
              }
      ...
      function errorHandlerFactory(downloadMeta) {
          return function (error) {
              ...
              if (state === 0 )
                  return;
              if (error) {
                  logger.error('Download error:', taskId, error);
              }
              else {
                  ...
                  if (ids.avaliable.length) {
                      for (const id of ids.avaliable) {
                          ...
                          parser
                              ...
                              .then((sources) => {
                              if (isCanceled) {
                                  logger.warn('Download stop manually: ' + id);
                                  return Promise.reject();
                              }
  ...
  function createFilter() {
      ...
      wrapper.appendChild(createFilterEl('pdl-filter-exclude_downloaded', 'filterExcludeDownloaded', i18n('dlbar_filter_exclude_downloaded')));
      wrapper.appendChild(createFilterEl('pdl-filter-illusts', 'filterIllusts', i18n('dlbar_filter_illusts')));
      wrapper.appendChild(createFilterEl('pdl-filter-manga', 'filterManga', i18n('dlbar_filter_manga')));
      wrapper.appendChild(createFilterEl('pdl-filter-ugoria', 'filterUgoria', i18n('dlbar_filter_ugoria')));
      return wrapper;
  }
  ...
  function updateDownloadBarBtnText(prevDlBtn, prevDlAllBtn) {
      if (location.pathname.includes('r18') && prevDlBtn.textContent !== i18n('dlbar.follow_latest.category_r18.single')) {
      ...
      else if (!location.pathname.includes('r18') &&
  ...
  function createFollowLatestDownloadBar() {
      ...
      if (prevDlBtn) {
          ...
          updateDownloadBarBtnText(prevDlBtn, prevDlAllBtn);
          return;
      }
  ...
  function createTabFilename({ tabName, paneHtml }) {
      ...
      if (env.isFileSystemAccessAvaliable()) {
          ...
          const conflictAction = config.get('fileSystemFilenameConflictAction');
          if (isUseFSA) {
              folder.placeholder = i18n('folder_tips');
          ...
          enableFSA.addEventListener('change', (ev) => {
              ...
              if (isEnabled) {
                  ...
                  if (env.isViolentmonkey()) {
                      folder.placeholder = i18n('folder_vm_tips');
                      folder.setAttribute('disabled', '');
                      folder.value = '';
                  }
                  else if (!env.isSupportSubpath()) {
                      folder.placeholder = i18n('folder_api_tips');
                      folder.setAttribute('disabled', '');
                      folder.value = '';
                  }
                  interactElems.forEach((el) => el.setAttribute('disabled', ''));
